...
一个工程一次快检，首三单 **人民币 99 元**。适用于微信小游戏体验版崩溃、Source Map 行列号不匹配、开发报错、准备审核/提审、已被驳回或不确定还缺哪些工程证据的项目；收到完整脱敏材料后 24 小时内交付 PDF 或 Markdown 风险报告，并含一次文字答疑。
...
- 开发者工具能运行，但体验版或真机报错；
- Cocos 本地 Source Map 与微信线上体验版错误坐标不一致；
- 准备审核/提审，不确定包体、隐私接口、存档或广告回调是否有阻断；
- 已被驳回，需要先区分工程问题、资质/类目问题和平台证据缺口；
- 本地测试很多，但不确定哪些证据能证明预览、真机、上传或提审状态。

## 检查范围

1. 工程入口、资源路径和包体风险；
2. 崩溃捕获入口、release、最终 game.js、Cocos 本地 map 与微信线上 map 的构建一致性；
3. 微信容器不兼容 API；
4. 隐私 API、用户数据、本地存档与清除路径；
5. 激励视频成功、失败、取消与重复回调风险；
6. 安全区、窄屏、触控与恢复路径；
7. 本地构建、开发者工具、官方预览、真机、上传、提审和上线证据是否混淆；
8. 最先应修复的三个 P0/P1 问题。
...
- 一页红/黄/绿结论摘要；
- 最优先的三个 P0/P1 阻断项；
- 逐项风险、代码或材料证据定位、优先级和修复建议；
- 预览、真机、上传、提审证据缺口清单；
- 一次文字答疑。

[查看虚构匿名示例](./sample-report.html)。示例只用于展示报告结构，不是真实客户案例，也不代表微信官方审核结果；[Markdown 源文件](./sample-report.md)保留用于审阅。
...
开始检查前只需要三类脱敏材料：工程技术栈与版本、问题/驳回现象的文字摘要、可供检查的脱敏工程副本或只读仓库。无法安全脱敏时不接收材料。
